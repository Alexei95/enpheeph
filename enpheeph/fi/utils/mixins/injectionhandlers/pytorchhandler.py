...
import enpheeph.fi.utils.dataclasses.binaryfaultmask
import enpheeph.fi.utils.enums.binaryfaultmaskop
...
class BinaryHandler(object):
    ...
    def generate_fault_mask(
            ...
            endianness: enpheeph.fi.utils.enums.endianness.Endianness,
            bit_value: enpheeph.fi.utils.enums.bitvalue.BitValue,
    ) -> enpheeph.fi.utils.dataclasses.binaryfaultmask.BinaryFaultMask:
        # if we are using little endian we invert the index, as the LSB is
        # at the end of the list
        if endianness == endianness.Little:
            bit_index = [
        ...
        if bit_value in (bit_value.StuckAtOne, bit_value.BitFlip):
            ...
            for index in bit_index:
                binary_mask[index] = '0'
            binary_mask_op = \
                    enpheeph.fi.utils.enums.binaryfaultmaskop.BinaryFaultMaskOp.AND
        else:
            raise ValueError('Unsupported injection type.')
        return enpheeph.fi.utils.dataclasses.binaryfaultmask.BinaryFaultMask(
