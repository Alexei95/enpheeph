...
MODEL_PATH = PYNNDORA_PATH / 'model_training/lightning/lenet5_mnist/default/version_5/checkpoints/epoch=25-step=9749.ckpt'

storage_plugin = (
        enpheeph.injections.plugins.picklestorageplugin.
        PickleStoragePlugin(
                path=FILE_DIR / 'test.pickle',
        )
)
pytorch_mask_plugin = (
        enpheeph.injections.plugins.cupypytorchmaskplugin.
        CuPyPyTorchMaskPlugin()
)
pytorch_handler_plugin = (
        enpheeph.handlers.plugins.pytorchhandlerplugin.
...
callback = enpheeph.extra.injectioncallback.InjectionCallback(
        injection_manager=injection_handler,
        storage_plugin=storage_plugin,
)

trainer = pytorch_lightning.Trainer(gpus=[2], callbacks=[callback])
datamodule = (
        pynndora.datasets.vision.mnist.MNISTDataModule(
                batch_size=4,
...
model = (
        pynndora.models.vision.lenet5.
        LeNet5.load_from_checkpoint(
                str(MODEL_PATH),
                strict=False,
        )
)
