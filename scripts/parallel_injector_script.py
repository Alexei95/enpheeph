...
TASK_MANAGER_OUTPUT_FILE_NAME_TEMPLATE = "task_manager_output.txt"
RESULT_DIRECTORY_TEMPLATE = "results_devices_{}_starting_seed_{}_random_threshold_{}"
...
async def run_injector_script(device, seed, namespace):
    result_directory = namespace.result_folder.absolute() / RESULT_DIRECTORY_TEMPLATE.format(namespace.devices, namespace.starting_seed, namespace.random_threshold) / ITERATION_RESULT_TEMPLATE.format(device, seed)
    ...
    command_args = f"{str(INJECTOR_SCRIPT_PATH)} --python-config {str(namespace.python_config.absolute())} --seed {seed} --target-csv {str(target_csv)} --number-iterations {iterations} --result-database {str(database)} --random-threshold {namespace.random_threshold}"
...
async def main(args=None):
    ...
    result_directory = namespace.result_folder / RESULT_DIRECTORY_TEMPLATE.format(namespace.devices, namespace.starting_seed, namespace.random_threshold)
    result_directory.mkdir(parents=True, exist_ok=True)
